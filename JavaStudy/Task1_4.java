 ...
 * Task1-4: 課題内容
 *
 * ログイン機能をイメージして処理を書いてみましょう！ ・アカウント情報としてデータベースに登録されている情報 名前：alice パスワード：alice123
...
public class Task1_4 {
	...
	public static void main(String[] args) {
		...
		else if (USER_NAME.equals(name) && !USER_PASSWORD.equals("123")) {
			System.out.println(CONST_MSG_ERROR_PASS);
		}
		// ④ 「USER_PASSWORD」の値のみ等しい場合。
		// 定数を使用して「 名前に誤りがあります。 」と出力してください。
		else if (!USER_NAME.equals("") && USER_PASSWORD.equals(pass)) {
			System.out.println(CONST_MSG_ERROR_NAME);
		}
		
		// ⑤ 「USER_NAME」も「USER_PASSWORD」の値も間違っていた場合。
		// 定数を使用して「 入力情報に誤りがあります。 」と出力してください。
		else (!USER_NAME.equals("") && !USER_PASSWORD.equals("")) {
